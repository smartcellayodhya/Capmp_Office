...
export function ExportButton({ officers, tierName = 'Officers' }: ExportButtonProps) {
  ...
  const handleExport = () => {
    ...
    const filename = `Ayodhya_Police_${cleanTier}_Report_${timestamp}.xlsx`
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-sm border border-emerald-600 transition-all duration-200 active:scale-95 group"
    >
      {downloaded ? (
        <>
