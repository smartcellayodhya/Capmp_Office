...
export default function NonGazettedDashboardPage() {
  ...
  // Fetch Non-Gazetted Officers with strict timeout safety
  const loadNonGazettedData = useCallback(async () => {
    setLoading(true)
    ...
    try {
      const fetchPromise = getOfficersByTier('Non-Gazetted')
      const result = await Promise.race([fetchPromise, timeoutPromise])

      if ('timeout' in result) {
        console.warn('Query Timed Out [Non-Gazetted Page]')
      ...
      if (error) {
        console.error('Error [Non-Gazetted Page]:', error.message, error)
  ...
  const handleDeleteSingle = async (pno: string) => {
    ...
    try {
      ...
      else if (success) loadNonGazettedData()
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="py-20 flex flex-col items-center justify-center gap-3 bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-xs text-slate-500 font-medium">Preparing Non-Gazetted Officers Cadre Roster</p>
        </div>
      ) : errorMessage ? (
        ...
        <>
          ...
          <Filters
            ...
            onBulkUploadClick={() => alert('Bulk Upload: Select a CSV/Excel file containing Non-Gazetted Officers columns (PNO, Name, Rank, Caste, Posting).')}
      ...
      {showAddModal && (
        <AddOfficerModal
          defaultTier="Non-Gazetted"
          onClose={() => setShowAddModal(false)}
          onSuccess={loadNonGazettedData}
        />
      )}
    </div>
  )
