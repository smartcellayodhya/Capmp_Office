...
import { getAllOfficers, getPostingApplications, getNodalOfficers } from '@/services/database'
import { enrichOfficerData } from '@/lib/policeUtils'
import { getCachedOfficers, setCachedOfficers, getCachedApps, setCachedApps } from '@/lib/cache'
...
export default function LiveDashboardPage() {
  ...
  // Fast fetch using Stale-While-Revalidate pattern (0ms Instant Load)
  const loadData = useCallback(async (isManualRefresh = false) => {
    if (isManualRefresh) setRefreshing(true)

    // Step 1: Check instant memory / localStorage cache first
    const cached = getCachedOfficers()
    const cachedApps = getCachedApps()

    if (cached && cached.length > 0) {
      setAllOfficers(cached)
      if (cachedApps) setPendingAppsCount(cachedApps.filter((a) => a.status === 'Pending').length)
